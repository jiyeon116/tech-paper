...
class SharedD3QNSession:
    """Own one TensorFlow graph/session for all independent UE D3QN agents.

    The simulator keeps an independent network and replay memory per UE.  This
    object changes only the TensorFlow execution layout: all UE networks are
    built in distinct variable scopes inside one graph and execute through one
    session.  This avoids one GPU context/allocation attempt per UE.
    """
    ...
    def _session_config():
        ...
        intra_op_threads = os.environ.get("TF_NUM_INTRAOP_THREADS")
        inter_op_threads = os.environ.get("TF_NUM_INTEROP_THREADS")
        if intra_op_threads:
            config.intra_op_parallelism_threads = int(intra_op_threads)
        if inter_op_threads:
            config.inter_op_parallelism_threads = int(inter_op_threads)
        return config

    def build_agent(self, scope, *args, **kwargs):
        if self.session is not None:
            raise RuntimeError("Cannot add D3QN agents after the shared session is initialized.")
        with self.graph.as_default():
    ...
    def initialize(self):
        if self.session is not None:
            return
        with self.graph.as_default():
            self.session = tf.Session(graph=self.graph, config=self._session_config())
...
class PerAgentD3QNSessions:
    """CPU-oriented execution runtime with one graph/session per independent UE.

    This preserves the legacy independent-session layout without creating GPU
    contexts. Use it only with CUDA disabled; the shared runtime is required
    for scalable GPU execution.
    """
    ...
    def build_agent(self, scope, *args, **kwargs):
        ...
        with graph.as_default():
            if self.seed is not None:
                tf.set_random_seed(self.seed + len(self.agents))
            agent = DuelingDoubleDeepQNetwork(*args, scope=scope, **kwargs)
        self.agents.append(agent)
        return agent

    def initialize(self):
        # Standalone DuelingDoubleDeepQNetwork instances initialize themselves.
        return None
...
class DuelingDoubleDeepQNetwork:
    ...
    def __init__(self,
        ...
        self.scope = scope or tf.get_default_graph().unique_name("d3qn")
        self._eval_collection = f"{self.scope}/eval_net_params"
        self._target_collection = f"{self.scope}/target_net_params"
        ...
        with tf.variable_scope(self.scope):
            ...
            # Replace only this UE's target-network parameters.
            t_params = tf.get_collection(self._target_collection)
            e_params = tf.get_collection(self._eval_collection)
            self.replace_target_op = [tf.assign(t, e) for t, e in zip(t_params, e_params)]

            own_trainable_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.scope)
            self.saver = tf.train.Saver(var_list=own_trainable_variables)

        self.sess = session
        if self.sess is None and initialize_session:
            self.sess = tf.Session(config=SharedD3QNSession._session_config())
            own_global_variables = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=self.scope)
            self.sess.run(tf.variables_initializer(own_global_variables))
        ...
        # Diagnostic-only history. It is not read by training or evaluation,
        # so bound it to prevent one entry per greedy action from accumulating
        # across all UE agents and episodes.
        self.store_q_value = deque(maxlen=256)

    def attach_session(self, session):
        if session.graph is not self.q_eval.graph:
            raise ValueError("The TensorFlow session graph does not match this D3QN agent graph.")
        self.sess = session
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            c_names, n_l1, n_lstm, w_initializer, b_initializer =  \
                [self._eval_collection, tf.GraphKeys.GLOBAL_VARIABLES], self.N_L1, self.N_lstm,\
        ...
        with tf.variable_scope('target_net'):
            c_names = [self._target_collection, tf.GraphKeys.GLOBAL_VARIABLES]
        ...
        with tf.variable_scope('train'):
            # In the shared graph, the default var_list would include every
            # UE's networks. Restrict backpropagation to this UE's eval net,
            # matching the original per-session behavior and avoiding an
            # O(number_of_UEs) variable scan for every new agent.
            self._train_op = tf.train.RMSPropOptimizer(self.lr).minimize(
                self.loss,
                var_list=tf.get_collection(self._eval_collection),
            )
